...
**音声再生は svhome02 のオーディオ端子から出力**。Home Assistant (192.168.0.21) が物理再生の責任を持ち、GoアプリのAPIレスポンスに含まれる `media_url` を元にHA自身が `media_player.play_media` を呼ぶ。Navidrome経由でFeishin/Symfoniumクライアントによるリモート再生も対応。管理画面は `https://news.futamura.dev` でCloudflare Tunnel経由で公開。

**音声再生の4経路:**
1. ATOM ECHO音声コマンド → HAオートメーション → HA media_player (`media_content_id: /media/tech/latest`) → svhome02オーディオ端子
2. Lovelace Dashboardボタン → HAスクリプト → HA media_player (`media_content_id: /media/tech/latest`) → svhome02オーディオ端子
3. 起床判定+モーションセンサ → HAオートメーション → HA media_player (`media_content_id: /media/tech/latest`) → svhome02オーディオ端子
...
- **物理音声出力: HAOSがai-newsの `/media/{category}/latest` を `media_content_id` として `media_player.play_media` を呼ぶ**（GoアプリからのHA逆コール不要・aplay/ffplay OS execも不要）
...
flowchart TD
    ...
    HA -->|GET /media/category/latest :8181| AINEWS
...
│   │   ├── playback_usecase.go       # 最新Broadcast取得 → media_url / title / duration_sec をJSONレスポンス用に返却（HA逆コールなし）
...
│   │       │                         # {category} をDB照合して最新broadcast取得 → media_url / title / duration_sec をJSONで返却（HA逆コールなし）
...
├── .env.example                      # GEMINI_API_KEY, NAVIDROME_*, SMB_HOST, SMB_USER, SMB_PASS 等
└── go.mod
```
...
erDiagram
    ...
    broadcasts {
        ...
        TEXT file_url "APIレスポンスのmedia_url / HAがmedia_content_idとして使用"
...
|---|---|---|---|---|
| `POST` | `/api/play/stop` | `api.PlayHandler.Stop` | 再生停止用プレースホルダ（exact matchのため `{category}` より優先）。ai-newsは `{"status":"ok"}` を返すのみ。実際の停止はHAが `media_player.media_stop` を直接呼ぶ | `{"status":"ok"}` |
| `POST` | `/api/play/{category}` | `api.PlayHandler.Play` | 指定カテゴリの最新broadcastを取得し、`media_url` / `title` / `duration_sec` を含むJSONを返す。HA逆コールなし。`{category}` をcategory_settings テーブルで照合（未知カテゴリは404）。HAがレスポンスの `media_url` を `media_content_id` として `media_player.play_media` を呼ぶ | `{"status":"ok","title":"...","media_url":"http://192.168.0.13:8181/media/tech/latest","broadcast_id":42,"duration_sec":300}` |
...
| `GET` | `/api/health` | `api.HealthHandler.Check` | ヘルスチェック（Docker用） | `{"status":"ok","db":"ok","voicevox":"ok"}` |
...
```yaml
# scripts.yaml（または configuration.yaml 内の script: セクション）
# ai-news が /media/{category}/latest を常時提供するため、HA から直接 play_media を呼ぶだけでよい
...
#   （再生自体は上記 script で実施。ai-news への HA_TOKEN 不要）
```

> **URL生成の二重化設計:**
> - `broadcasts.file_url` (DBに保存): `APP_BASE_URL` (LAN IP) ベースのURL → `/api/play/{category}` のJSONレスポンス内 `media_url` として返却。HAがこの値を `media_content_id` として使用。HAは常にLAN内なのでLAN URLで問題なし
...
GoアプリはOS execでの再生も HA への逆コールも行わない。HAが `media_content_id` を使って自律的に再生する。
...
sequenceDiagram
    ...
    Note over HA,GoApp: HA が直接 media_player.play_media を呼ぶ<br/>media_content_id: http://192.168.0.13:8181/media/tech/latest

    HA->>GoApp: GET /media/tech/latest
...
```mermaid
flowchart TD
    subgraph PLAY[POST /api/play/category （メタ情報取得用）]
        P1["category_settings で照合<br/>未知カテゴリ → 404"] --> P2["broadcastRepo.GetLatest<br/>最新broadcast取得"]
        P2 --> P3["200 JSONレスポンス返却<br/>media_url / title / duration_sec / broadcast_id"]
    end
...
services:
  ai-news:
    ...
    environment:
      ...
      - APP_BASE_URL=http://192.168.0.13:8181    # broadcasts.file_url 生成用 LAN URL（HAがmedia_content_idとして使用）
...
5. **音声再生テスト** (curlで /api/play メタ情報確認、HAで実再生)
   ```bash
   curl -X POST http://192.168.0.13:8181/api/play/tech
   # → {"status":"ok","title":"...","media_url":"http://192.168.0.13:8181/media/tech/latest","broadcast_id":42,"duration_sec":300}
   # レスポンスに media_url が含まれることを確認
   curl -X POST http://192.168.0.13:8181/api/play/unknown-category
   # → 404 （未知カテゴリは拒否されることを確認）
   # HA の開発者ツール → サービスコール: media_player.play_media
   #   media_content_id: "http://192.168.0.13:8181/media/tech/latest" → svhome02オーディオ端子から再生
   ```

6. **メディア配信テスト** (ブラウザまたはcurlで)
   ```
   http://192.168.0.13:8181/media/tech/latest
   # go-smb2経由でSMBから読み出しストリーム配信されることを確認（Content-Type: audio/mpeg）
   ```
