...
│   ├── partials/
│   │   ├── article_list.html         # HTMX swapターゲット: 記事カードリスト
...
| `POST` | `/ui/pipeline/trigger` | `web.SystemHandler.TriggerPipeline` | 手動ジョブ実行（`?job_type=scrape\|generate`）→ run_idをHTMXに返す |
...
flowchart TD
    ...
    D --> E["articleRepo.NullifyRawContent<br/>raw_content NULLクリア<br/>DBサイズ削減"]
    E --> F["articleRepo.DeleteOlderThan<br/>古い記事レコード削除<br/>pipeline_runsはcleanup対象外"]
```
...
services:
  ...
  playwright-chrome:
    image: ghcr.io/browserless/chromium:latest  # CDP WebSocketエンドポイントを提供
